    ...
    <div className="p-6 h-full flex flex-col space-y-6 relative">
      ...
      <div className="bg-white p-5 rounded-xl border border-slate-200 shadow-sm space-y-4">
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-5 gap-4">
            <div className="lg:col-span-2"><label className="text-xs font-bold text-slate-500 mb-1 block">ค้นหา (Search)</label><div className="relative"><Search className="absolute left-3 top-1/2 -translate-y-1/2 text-slate-400 w-4 h-4" /><input type="text" placeholder="ค้นหา Ref, NCR, สินค้า, ลูกค้า..." value={filters.query} onChange={(e) => setFilters({...filters, query: e.target.value})} className="w-full pl-9 pr-4 py-2 bg-slate-50 border border-slate-200 rounded-lg focus:ring-2 focus:ring-blue-500 outline-none text-sm" /></div></div>
                            ...
                            <div className="space-y-3 text-sm">
                              ...
                              <div>
                                ...
                                <div className="space-y-2 text-xs bg-slate-50 p-3 rounded-lg border border-slate-100">
                                  ...
                                  {selectedItem.actionScrapReplace && (<p><span className="font-bold w-32 inline-block">เปลี่ยนสินค้าใหม่:</span> {selectedItem.actionScrapReplaceQty} units</p>)}
        ...
        <div className="fixed inset-0 z-[100] bg-black/60 flex items-center justify-center p-4">
          <div className="bg-white w-full max-w-lg rounded-xl shadow-2xl flex flex-col max-h-[90vh]">
            ...
            <div className="p-4 overflow-auto space-y-3">
              <div><label className="text-xs">Branch</label><input type="text" value={editingItem.branch} onChange={e => setEditingItem({...editingItem, branch: e.target.value})} className="w-full border p-1 rounded" /></div>
              <div><label className="text-xs">Customer Name</label><input type="text" value={editingItem.customerName} onChange={e => setEditingItem({...editingItem, customerName: e.target.value})} className="w-full border p-1 rounded" /></div>
              <div><label className="text-xs">Product Name</label><input type="text" value={editingItem.productName} onChange={e => setEditingItem({...editingItem, productName: e.target.value})} className="w-full border p-1 rounded" /></div>
              <div><label className="text-xs">Quantity</label><input type="number" value={editingItem.quantity} onChange={e => setEditingItem({...editingItem, quantity: parseInt(e.target.value)})} className="w-full border p-1 rounded" /></div>
              <div><label className="text-xs">Price (Bill)</label><input type="number" value={editingItem.priceBill} onChange={e => setEditingItem({...editingItem, priceBill: parseFloat(e.target.value)})} className="w-full border p-1 rounded" /></div>
              <div><label className="text-xs">Date</label><input type="date" value={editingItem.date} onChange={e => setEditingItem({...editingItem, date: e.target.value})} className="w-full border p-1 rounded" /></div>
              <div><label className="text-xs">Notes</label><textarea value={editingItem.notes} onChange={e => setEditingItem({...editingItem, notes: e.target.value})} className="w-full border p-1 rounded" /></div>
            </div>
