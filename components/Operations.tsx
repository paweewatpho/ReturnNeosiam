...
const Operations: React.FC<OperationsProps> = ({ initialData, onClearInitialData }) => {
  ...
  // NEW: State for Step 4 item selection
  const [step4SelectedIds, setStep4SelectedIds] = useState<Set<string>>(new Set());

  // Print Modal States
  const [showPrintModal, setShowPrintModal] = useState(false);
  const [selectedItemsForPrint, setSelectedItemsForPrint] = useState<Set<string>>(new Set());
  ...
  const handleOpenPrintModal = () => {
    ...
    setSelectedItemsForPrint(new Set());
  };
  ...
  const handlePrintSelected = () => {
    if (selectedItemsForPrint.size === 0) {
  ...
  const handleQCSubmit = async () => {
    ...
    if (success) {
        ...
        setCustomInputType(null);
        setIsCustomRoute(false);
        alert('บันทึกผลการตรวจสอบคุณภาพเรียบร้อย (Ready for Next Step)');
    }
  };

  const toggleStep4Selection = (id: string) => {
    setStep4SelectedIds(prev => {
        const newSet = new Set(prev);
        if (newSet.has(id)) {
            newSet.delete(id);
        } else {
  ...
  const handleConfirmStep4Items = async () => {
    if (step4SelectedIds.size === 0) {
        alert("กรุณาเลือกรายการสินค้าเพื่อดำเนินการต่อ");
        return;
    }

    if (!window.confirm(`คุณต้องการยืนยันการดำเนินการสำหรับ ${step4SelectedIds.size} รายการที่เลือกใช่หรือไม่?`)) {
        return;
    }

    const today = new Date().toISOString().split('T')[0];
    const promises = Array.from(step4SelectedIds).map(id => 
        updateReturnRecord(id, { status: 'Documented', dateDocumented: today })
    );
    ...
    try {
        ...
        if (successCount > 0) {
            alert(`ดำเนินการ ${successCount} จาก ${step4SelectedIds.size} รายการสำเร็จ! กำลังนำทางไปยังขั้นตอนถัดไป`);
            // Clear all selections as we are moving away
            setStep4SelectedIds(new Set());
            // *** THE KEY CHANGE: AUTO-NAVIGATE TO STEP 5 ***
            setActiveStep(5);
        } else {
            alert("การดำเนินการล้มเหลวทั้งหมด กรุณาตรวจสอบสิทธิ์การใช้งานหรือการเชื่อมต่อ");
        }
    } catch (error) {
        alert("เกิดข้อผิดพลาดในการอัปตเดตข้อมูล");
        console.error("Step 4 confirmation error:", error);
    }
  };
  ...
  const KanbanColumn = ({ title, status, icon: Icon, color, selectedIds, onSelect }: { title: string, status: DispositionAction, icon: any, color: string, selectedIds: Set<string>, onSelect: (id: string) => void }) => {
    ...
    return (
      <div className="flex-1 min-w-[280px] bg-slate-50 rounded-xl flex flex-col h-full border border-slate-200">
        ...
        <div className="p-2 space-y-2 overflow-y-auto flex-1">
           {list.map(item => (
             <label key={item.id} className={`block bg-white p-3 rounded-lg border shadow-sm transition-all cursor-pointer relative ${selectedIds.has(item.id) ? 'border-blue-500 ring-2 ring-blue-200' : 'border-slate-100 hover:border-blue-200'}`}>
                <input
                    type="checkbox"
                    checked={selectedIds.has(item.id)}
                    onChange={() => onSelect(item.id)}
                    className="absolute top-2 right-2 w-5 h-5 text-blue-600 rounded focus:ring-blue-500"
                />
                <div className="flex justify-between items-start mb-1"> <span className="text-xs font-mono text-slate-400">{item.id}</span></div>
                <div className="text-xs text-blue-600 mb-0.5">{item.branch}</div>
                <h4 className="text-sm font-semibold text-slate-800 truncate pr-6">{item.productName}</h4>
           ...
           {list.length === 0 && ( <div className="text-center p-4 text-slate-400 text-xs italic">ไม่มีรายการรอ</div> )}
        </div>
      </div>
    );
  };
    ...
    <div className="h-full flex flex-col bg-white relative">
      <div className="border-b border-slate-200 px-6 pt-4 bg-white shadow-sm z-10">
        <div className="flex gap-2 overflow-x-auto pb-0">
           ...
           <button onClick={() => setActiveStep(4)} className={`pb-3 text-sm font-bold flex items-center gap-2 border-b-2 transition-colors whitespace-nowrap px-4 ${activeStep === 4 ? 'border-blue-600 text-blue-600' : 'border-transparent text-slate-500 hover:text-slate-700'}`}> <span className={`w-6 h-6 rounded-full flex items-center justify-center text-xs ${activeStep === 4 ? 'bg-blue-600 text-white' : 'bg-slate-200 text-slate-600'}`}>4</span> ดำเนินการ (Execution) {gradedItems.length > 0 && <span className="bg-amber-500 text-white text-[10px] px-1.5 rounded-full">{gradedItems.length}</span>} </button>
      ...
      <div className="flex-1 overflow-hidden bg-slate-50/50">
        ...
        {activeStep === 4 && (
            <div className="h-full overflow-x-auto p-4 flex gap-4 relative">
                <KanbanColumn title="สินค้าสำหรับส่งคืน (RTV)" status="RTV" icon={Truck} color="bg-amber-500" selectedIds={step4SelectedIds} onSelect={toggleStep4Selection} />
                <KanbanColumn title="สินค้าสำหรับขาย (Restock)" status="Restock" icon={RotateCcw} color="bg-green-500" selectedIds={step4SelectedIds} onSelect={toggleStep4Selection} />
                <KanbanColumn title="สินค้าสำหรับเคลม (Claim)" status="Claim" icon={ShieldCheck} color="bg-blue-500" selectedIds={step4SelectedIds} onSelect={toggleStep4Selection} />
                <KanbanColumn title="สินค้าใช้ภายใน (Internal)" status="InternalUse" icon={Home} color="bg-purple-500" selectedIds={step4SelectedIds} onSelect={toggleStep4Selection} />
                <KanbanColumn title="สินค้าสำหรับทำลาย (Scrap)" status="Recycle" icon={Trash2} color="bg-red-500" selectedIds={step4SelectedIds} onSelect={toggleStep4Selection} />
                
                {step4SelectedIds.size > 0 && (
                    <div className="fixed bottom-6 right-6 z-20">
                        <button 
                            onClick={handleConfirmStep4Items}
                            className="px-6 py-4 bg-green-600 text-white font-bold rounded-full shadow-lg flex items-center gap-2 hover:bg-green-700 transform hover:scale-105 transition-all"
                        >
                            <CheckSquare className="w-5 h-5" /> ยืนยันการดำเนินการ ({step4SelectedIds.size} รายการ)
                        </button>
                    </div>
                )}
            </div>
